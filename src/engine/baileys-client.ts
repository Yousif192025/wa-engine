...
export interface BaileysTransport {
  ...
  status(): { status: WhatsAppConnectionStatus; connected: boolean; jid?: string }
}
...
export class BaileysClient implements BaileysTransport {
  ...
  async stop(): Promise<void> {
    ...
    this.sessionRepository.saveConnectionState({ status: 'disconnected', disconnected: true })
      .catch((error) => logError('whatsapp_connection_state_save_failed', error))
  }
  ...
  async sendText(jid: string, message: string): Promise<string | undefined> {
  ...
  private async handleConnectionUpdate(
    ...
    if (update.qr && !shouldUsePairingCode) {
      ...
      try {
        ...
        logError('whatsapp_qr_render_failed', error)
      }
    }
    ...
    if (loggedOut) {
      ...
      await this.sessionRepository.saveConnectionState({
        status: 'logged_out',
        disconnected: true,
        lastError: 'WhatsApp logged out; scan a new QR code to reconnect.',
      })
  ...
  private handleInboundMessage(message: WAMessage): void {
    ...
    log('whatsapp_message_received', { messageId: message.key.id, chat: message.key.remoteJid })
    this.messageHandler(message).catch((error) => {
      logError('whatsapp_message_processing_failed', error, { messageId: message.key.id })
    })
  }
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      this.connect().catch((error) => {
        logError('whatsapp_reconnect_failed', error)
        this.scheduleReconnect()
      })
